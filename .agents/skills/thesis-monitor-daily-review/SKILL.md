...
6. Write one complete draft JSON document to the returned claim-specific `temp_output_path`. Set `claim_id` from the claim response and copy both Knowledge versions and checksums. Every `numeric_fact_refs[].ref_id` must be unique within its review, its placeholder must occur exactly once in its declared `text_ref`, and its `fact_id` must also be in `facts_used`. The binder converts the draft to the final schema-4 document. Do not write outside `data/ai_review`.
   A numeric placeholder represents the complete canonical phrase: source/instrument-aware label plus formatted value. Write `{{numeric:pe}}보다 {{numeric:fpe}}가 높습니다`, not `현재 PER {{numeric:pe}}보다 선행 PER {{numeric:fpe}}가 높습니다`. Likewise write `{{numeric:revenue}}와 {{numeric:revenue_yoy}}`, not `매출 {{numeric:revenue}}와 매출 성장률 {{numeric:revenue_yoy}}`. The binder owns `현재 PER`, `시장 예상 fPER`, `내부 추정 fPER`, index names, and night-futures product names. General context such as `TWD 기준인 {{numeric:revenue}}를 ADR 가격과 환산하지 않습니다` is allowed because it does not pre-author the numeric label.
...
- Security identity fails closed independently of financial quality. Use security/share-basis-dependent valuation only when `security_identity:current` and the associated registry rows are eligible. `conflict` and `unknown` are not verified non-depositary states: do not cite the denied multiple, restate its value, infer growth/cheapness/expensiveness from it, estimate an ADR ratio, or perform a currency conversion. Explain the concrete identity or share-basis gap through `security_identity:current`, and continue with independent price, chart, volume, supply, or issuer-level financial facts that remain eligible.
