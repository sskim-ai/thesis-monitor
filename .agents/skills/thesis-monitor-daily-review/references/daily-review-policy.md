...
KR investor-flow language is actor-and-horizon exact. A directional statement for foreign or institutional flow at 1, 5, or 20 days requires that exact canonical claim in the same prose occurrence; another actor or horizon cannot substitute. US stock prose uses volume, relative volume, or an explicit US positioning fact and does not use generic `수급` language without such a contract. Every displayed financial amount and directly attached growth or margin claim carries a field-specific amount-period label derived from one uniquely matched source row. Filing type and report end date never substitute for single-quarter/cumulative status, consolidated/separate basis, or comparison-period identity. Historical, peer, expectation, and trailing/forward valuation judgments require a typed draft interpretation reference and their matching visible comparison claims. A trailing/forward relation is usable only when the backend relation fact verifies the same price, security, share, currency, and denominator basis. PBR/BVPS interpretation must pass `valuation-coherence-v1` on period, currency, share, and security basis.
...
After rendering, `runtime-message-quality-v1` must pass before the AI payload can be persisted for delivery. Its version-2 receipt binds the packet, validated output, complete rendered payload set, policy, schema, message count, check results, errors, and status. Delivery metadata stores the SHA-256 of the complete receipt file and the rendered payload-set hash. A missing, invalid, or changed receipt is a quality-integrity rejection, not a network retry: the AI payload is withheld, no receipt is regenerated, and the already persisted deterministic fallback remains eligible for exactly one deadline delivery. Existing completed archives remain governed by their historical manifest and are never rewritten merely because they predate the receipt contract.
