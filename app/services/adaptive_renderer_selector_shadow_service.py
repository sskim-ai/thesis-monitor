...
CONTRACT_VERSION = "adaptive-renderer-selector-shadow-v1"
            ...
            "rendered": (
                {
                    ...
                    "sentence_supports": [
                        asdict(row) for row in self.rendered.sentence_supports
                    ],
                }
                if self.rendered
                else None
...
Renderer = Callable[
    [str, FreeAnalystAnalysis, AdaptiveRenderer], RenderedFreeAnalyst
]
...
def _unique_material_warning(analysis: FreeAnalystAnalysis) -> bool:
    ...
    return any(
        _normalized(row.unresolved_question) not in next_checks
        for row in analysis.unknowns
    )
...
def _available_elements(
    ...
    return {
        ...
        InformationElement.ALTERNATIVE_INTERPRETATION: bool(
        ...
        InformationElement.EXPECTATION_VALUATION: bool(
            analysis.expectation_valuation_interaction
        ),
        InformationElement.POSITIONING_SYNTHESIS: bool(
...
def _material_elements(
    ...
    return {
        InformationElement.PRIMARY_CONCLUSION: available[
            InformationElement.PRIMARY_CONCLUSION
        ],
        InformationElement.THESIS_LINKAGE: available[
            InformationElement.THESIS_LINKAGE
        ],
        InformationElement.ALTERNATIVE_INTERPRETATION: available[
            InformationElement.ALTERNATIVE_INTERPRETATION
        ],
        InformationElement.UNCERTAINTY_BOUNDARY: available[
            InformationElement.UNCERTAINTY_BOUNDARY
        ],
        ...
        InformationElement.MATERIAL_WARNING: available[
            InformationElement.MATERIAL_WARNING
        ],
    }
...
def _support_retention(
    ...
    retained_items = {
        ...
        if item.item_id in support_ids
    }
    ...
    boundary_ids = {
        item.item_id for item in analysis.analysis_items() if item.boundary
    }
    return {
        ...
        InformationElement.UNCERTAINTY_BOUNDARY: bool(
            boundary_ids.intersection(support_ids)
        ),
        InformationElement.EXPECTATION_VALUATION: any(
            item.item_id in support_ids
            for item in analysis.expectation_valuation_interaction
        ),
...
def renderer_information_audit(
    ...
    if renderer == AdaptiveRenderer.DIRECT_ANALYST:
        ...
        retained = _support_retention(
            analysis, render_free_analyst_vnext_hybrid(analysis)
        )
    else:
        retained = _minimal_retention(
            analysis, build_vnext_shadow_message(current_ai_text).text
        )
    ...
    material_dropped = tuple(
        element for element in dropped_elements if material[element]
    )
...
def select_adaptive_renderer(
    ...
    if by_renderer[
        AdaptiveRenderer.CONCISE_HYBRID
    ...
    elif _low_information_shape(analysis):
...
def render_adaptive_candidate(
    ...
    minimal = build_vnext_shadow_message(current_ai_text)
...
def _fallback_result(
    *,
    ...
    minimal = build_vnext_shadow_message(current_ai_text)
    parity = factual_parity_report(current_ai_text, minimal.text)
    if parity["status"] == "PASS":
        text = minimal.text
        mode = "VNEXT_SHADOW_FALLBACK"
    else:
        text = deterministic_reference or current_ai_text
        mode = "DETERMINISTIC_SHADOW_FALLBACK"
...
def run_adaptive_renderer_shadow(
    current_ai_text: str,
    *,
    benchmark_id: str,
    deterministic_reference: str = "",
    ...
    safety = adaptive_renderer_safety_report(
        current_ai_text, analysis, decision, rendered
    )
    ...
    return AdaptiveShadowResult(
        ...
        final_delivery_mode="ADAPTIVE_SHADOW_WOULD_SEND",
