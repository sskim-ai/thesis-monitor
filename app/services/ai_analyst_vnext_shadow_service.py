...
CONTRACT_VERSION = "ai-analyst-vnext-shadow-v1"
...
def _stock_vnext(parsed: ParsedMessage) -> VNextShadowResult:
    ...
    holder_value = (
        _named_view(price_rows[0].body, "보유자:") if price_rows else ""
    )
    valuation_value = (
        _least_numeric_sentence(valuation_rows[0].body) if valuation_rows else ""
    )
    price_valuation = [
        value for value in (price_value, holder_value, valuation_value) if value
    ]
    if price_valuation:
    ...
    next_items, duplicates = _dedupe_items(
        *(row.body for row in (*next_rows, *unknown_rows))
    )
    if next_items:
...
def _market_vnext(parsed: ParsedMessage) -> VNextShadowResult:
    blocks: list[tuple[str, list[str]]] = []
    selected: list[str] = []
    emitted_keys: set[str] = set()
...
def build_vnext_shadow_message(current_ai_text: str) -> VNextShadowResult:
...
def factual_parity_report(current_ai_text: str, vnext_text: str) -> dict[str, object]:
    ...
    unsupported_numbers = sorted(
        token
        for token, count in vnext_numbers.items()
...
def advisory_value_add_gate(
    ...
    advisory_checks = {
        ...
        "excessive_numeric_recitation": (
            "PASS" if vnext_density <= current_density else "FAIL"
        ),
        "supported_value_add_operation": (
            "PASS" if result.value_add_types else "FAIL"
        ),
        "deterministic_like_paraphrase_only": (
            "PASS" if materially_different else "FAIL"
        ),
    }
    return {
        "contract": CONTRACT_VERSION,
        ...
        "duplicate_next_check_unknown_before": (
            result.duplicate_next_check_unknown_before
        ),
        "duplicate_next_check_unknown_after": (
            result.duplicate_next_check_unknown_after
        ),
