...
def _bank_insurer_subframework(
    ...
    text = _framework_text(
        ...
        profile_payload.get("legal_name"),
    )
    if any(
        token in text
        for token in ("bank", "financial_holding", "은행", "금융지주")
    ):
        return "bank"
...
def _opendart_common_fields(
    filing: Filing, lineage: Mapping[str, object]
) -> dict[str, object]:
    return {
...
def kr_bank_insurer_sector_fact(
    *,
    ticker: str,
    ...
    specs = (
        ...
        else _KR_INSURANCE_SECTOR_SPECS
    )
    ...
    for spec in specs:
        ...
        metrics.append(
            {
                "metric_family": spec.logical_field,
                "account_id": lineage.get("account_id"),
                "account_name": lineage.get("account_name"),
                "value": lineage.get("amount"),
                "currency": lineage.get("currency"),
                "period_type": lineage.get("amount_period_type"),
                "period_start": lineage.get("amount_period_start"),
                "period_end": lineage.get("amount_period_end"),
...
class OfficialFundamentalEnricher:
    ...
    async def _enrich_kr(
        ...
        *,
        ...
        revenue_lineage = (
            revenue.get("lineage")
            if isinstance(revenue.get("lineage"), Mapping)
            else {}
        )
        earnings_lineage = next(
            (
                row.get("lineage")
                for row in (operating, net_income)
                if row.get("status") == "verified_usable"
                and isinstance(row.get("lineage"), Mapping)
            ),
            {},
        )
        if verified_revenue:
            facts.append(
                _official_fact(
                    ...
                    source_payload_sha256=source_sha,
                    fields={
                        **_opendart_common_fields(filing, revenue_lineage),
                        "reported_revenue": revenue.get("value"),
                        "source_row_identity": revenue_lineage.get(
                            "source_row_identity"
                        ),
                    },
                )
        ...
        if verified_earnings:
            ...
            facts.append(
                _official_fact(
                    ...
                    source_payload_sha256=source_sha,
                    fields={
                        **_opendart_common_fields(filing, earnings_lineage),
                        **earnings_fields,
                        "source_row_identity": earnings_lineage.get(
                            "source_row_identity"
                        ),
                    },
                )
        ...
        if sector_fact is not None:
            facts.append(sector_fact)
        verified_sector = sector_fact is not None
        if framework == AnalysisFramework.BANK_INSURER:
            quality = (
                FundamentalEvidenceQuality.CURRENT
                if verified_earnings and verified_sector
                else FundamentalEvidenceQuality.PARTIAL
                if verified_earnings or verified_sector
        ...
        verified_financial = verified_revenue or verified_earnings or verified_sector
        return OfficialFundamentalEnrichment(
            ticker=item.ticker,
            market="kr",
            issuer_id=profile.corp_code,
            ...
            source_attempts=(
                ...
                {
                    "source": "opendart_formal_statement",
                    "status": "success" if verified_financial else "unavailable",
                    "source_document_id": filing.receipt_no,
                },
            ),
            errors=errors,
            provider_audit={
                ...
                "profile_successes": 1,
                "statement_requests": client.provider_calls,
                "statement_successes": int(verified_financial),
