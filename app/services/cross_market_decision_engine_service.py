...
_FACT_TYPE_MODEL_LABELS = {
    "cash_flow_fcf_ppe": "cash_conversion_ocf_less_ppe",
}


def _model_facing_label_for_fact(row: Mapping[str, object]) -> str:
    fact_type = str(row.get("fact_type") or "").lower()
    return _FACT_TYPE_MODEL_LABELS.get(
        fact_type,
        str(row.get("fact_type") or row.get("fact_id") or "financial_fact"),
    )
