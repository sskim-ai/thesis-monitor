...
CONTRACT_VERSION = "evidence-locked-free-analyst-shadow-v1"
...
_RULE_REQUIRED_SECTIONS: dict[InferenceRule, frozenset[str]] = {
    ...
    InferenceRule.CONTRACT_ASSET_RECOVERY_GAP: frozenset(
        {"core", "business", "next_check"}
    ),
    InferenceRule.FLEET_INVESTMENT_RECOVERY_GAP: frozenset(
        {"core", "business", "next_check"}
    ),
    ...
    InferenceRule.MEMORY_CYCLE_FCF: frozenset(
        {"metadata", "core", "business", "next_check"}
    ),
    InferenceRule.EXPECTATION_VERIFICATION_THRESHOLD: frozenset(
        {"metadata", "core", "next_check"}
    ),
...
def build_evidence_catalog(current_ai_text: str) -> tuple[EvidenceAtom, ...]:
    ...
    for line in _content_lines(parsed.preamble):
        metadata_index += 1
        atoms.append(
            EvidenceAtom(f"{prefix}:metadata:{metadata_index:02d}", "metadata", line)
        )
...
def build_free_analyst_analysis(
    ...
    *,
    ...
    common_refs = tuple(
        dict.fromkeys(
            (*_expectation_ref(catalog), *core_refs, *business_refs, *next_refs)
        )
    )
...
def _rule_sections(
    ...
    return frozenset(
        catalog[ref].section_key for ref in item.evidence_refs if ref in catalog
    )
    ...
    for item in analysis.analysis_items():
        ...
        if item.support_type in {SupportType.DIRECT_FACT, SupportType.DIRECT_RELATION}:
            if item.text not in source:
                issues.append(
                    ValidationIssue(
                        "direct_claim_not_source_span", item.item_id, item.text
                    )
                )
        else:
        ...
        if item_numbers and item.support_type not in {
            ...
            issues.append(
                ValidationIssue("unsupported_numeric_claim", item.item_id, item.text)
            )
        ...
        if _CAUSAL_OVERREACH.search(item.text) or (
            ...
            issues.append(
                ValidationIssue("unsupported_causal_conclusion", item.item_id, item.text)
            )
        external_tokens = {
            token
        ...
        if (
            item.support_type == SupportType.POSITIONING_SYNTHESIS
            and re.search(r"(?:투자 논리|사업).*(?:약화|훼손)", item.text)
        ):
            issues.append(
                ValidationIssue("supply_fundamental_promotion", item.item_id, item.text)
            )
...
def render_free_analyst_direct(analysis: FreeAnalystAnalysis) -> RenderedFreeAnalyst:
    ...
    if analysis.expectation_valuation_interaction:
        blocks.append(
            ("💰 기대·Valuation", [analysis.expectation_valuation_interaction[0]])
        )
...
def render_free_analyst_vnext_hybrid(
    ...
    if analysis.expectation_valuation_interaction:
        ...
        blocks.append(
            ("⚖️ 경계", [analysis.alternative_interpretations[0].negative_interpretation])
        )
...
def rendered_safety_report(
    ...
    unsupported_numbers = sorted(
        token
        for token, count in rendered_numbers.items()
        if count > current_numbers.get(token, 0)
    )
...
def novel_synthesis_report(
    ...
    return {
        ...
        "novel_supported_synthesis_sentences": (
            novel if safety["status"] == "PASS" else 0
        ),
