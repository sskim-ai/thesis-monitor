...
CONTRACT_VERSION = "financial-framework-claim-scope-v2"
CLAIM_SPAN_CONTRACT_VERSION = "financial-framework-claim-span-v1"
...
class FrameworkClaim:
    ...
    local_clause_start: int = -1
    local_clause_end: int = -1
    span_contract: str = CLAIM_SPAN_CONTRACT_VERSION


@dataclass(frozen=True)
class LocalFinancialClaimSpan:
    """Provider-neutral local clause surrounding one financial term."""
...
_LOCAL_CLAUSE_BOUNDARY = re.compile(
    r"[,，:：]+|"
    r"(?:이며|이고)\s+|"
    r"\s+(?:그리고|이며|이고|하지만|그러나|반면|다만)\s+|"
    r"\s+(?:and|but|however|while)\s+",
    re.I,
)
_NOMINAL_BRIDGE = (
...
_KO_CONNECTIVE_EXCLUSION = re.compile(
    _NOMINAL_BRIDGE
    + r"(?P<predicate>"
    r"(?:배제|제외)\s*하고|"
    r"(?:적용|사용|평가|활용)\s*하지\s*않고|"
    r"보지\s*않고|"
    r"(?:배제|제외)\s*한\s*채"
    r")\s*,?\s*(?P<replacement>.+)$"
)
_KO_CONNECTIVE_REPLACEMENT_APPLICATION = re.compile(
    r"(?:(?!하지만|그러나|반면).)+?"
    r"(?:을|를|이|가|으로|로)?\s*"
    r"(?:(?:중심|기준)으로\s*)?"
    + _KO_REPLACEMENT_APPLICATION_VERB
...
def _ko_connective_replacement_exclusion(suffix: str) -> bool:
    match = _KO_CONNECTIVE_EXCLUSION.fullmatch(suffix)
    if match is None:
        return False
    ...
    return bool(
        replacement
        and _SECTOR_VALID_REPLACEMENT.search(replacement)
        and _KO_CONNECTIVE_REPLACEMENT_APPLICATION.fullmatch(replacement)
        and _CLUSTER.search(replacement) is None
    )
...
def _contrastive_replacement_exclusion(*, prefix: str, suffix: str, clause: str) -> bool:
    if _CONDITIONAL.search(clause):
        return False
    if _ko_contrastive_replacement(suffix) or _ko_connective_replacement_exclusion(
        suffix
    ):
        return True
...
def _local_clause_span(
    ...
    *,
    ...
    left = 0
    right = len(text)
    for boundary in _LOCAL_CLAUSE_BOUNDARY.finditer(text):
        if boundary.end() <= cluster_start:
            left = boundary.end()
            continue
        if boundary.start() >= cluster_end:
...
def local_financial_claim_spans(
    text: str,
    *,
    term_pattern: re.Pattern[str],
) -> tuple[LocalFinancialClaimSpan, ...]:
    """Return the established financial clause boundary for each matched term."""

    normalized = unicodedata.normalize("NFKC", text)
    spans: list[LocalFinancialClaimSpan] = []
    seen: set[tuple[int, int, int, int]] = set()
    for sentence in _SENTENCE.finditer(normalized):
        clause = sentence.group()
        for term in term_pattern.finditer(clause):
            local_text, local_start, local_end = _local_clause_span(
                clause,
                cluster_start=term.start(),
                cluster_end=term.end(),
            )
            absolute_local_start = sentence.start() + local_start
            absolute_local_end = sentence.start() + local_end
            absolute_term_start = sentence.start() + term.start()
            absolute_term_end = sentence.start() + term.end()
            identity = (
            ...
            seen.add(identity)
            spans.append(
                LocalFinancialClaimSpan(
                    full_field_text=normalized,
                    local_clause_text=local_text,
                    local_clause_start=absolute_local_start,
                    local_clause_end=absolute_local_end,
                    term_text=term.group(),
                    term_start=absolute_term_start,
                    term_end=absolute_term_end,
                )
            )
    return tuple(spans)
                ...
                if re.search(pattern, cluster.group(), re.I):
                    claims.append(
                        FrameworkClaim(
                            ...
                            full_field_text=normalized,
                            local_clause_text=local_text,
                            local_clause_start=sentence.start() + local_start,
                            local_clause_end=sentence.start() + local_end,
                        )
                    )
    return tuple(claims)
                ...
                if key in _TEXT_FIELDS and isinstance(value, str):
                    ...
                    claims.extend(
                        FrameworkClaim(
                            ...
                            evidence_refs=refs,
                            role=scoped_role(c, child_path, refs),
                            full_field_text=c.full_field_text,
                            local_clause_text=c.local_clause_text,
                            local_clause_start=c.local_clause_start,
                            local_clause_end=c.local_clause_end,
                            span_contract=c.span_contract,
                        )
...
def financial_frameworks_in_text(text: str) -> frozenset[str]:
    """Return explicitly named canonical financial frameworks."""

    return frozenset(claim.framework for claim in financial_framework_claims(text))
