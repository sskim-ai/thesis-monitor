...
CONTRACT_VERSION = "free-analyst-natural-packet-adapter-v1"
...
def _content_fingerprint(value: str) -> str:
    content = "\n".join(
        line for line in value.splitlines() if not line.strip().startswith(_HEADING_PREFIXES)
    )
...
def _evidence_ref_map(
    ...
    *,
    ...
    for line in _content_lines(parsed.preamble):
        ...
        refs.append(
            AdapterEvidenceRef(
                production_ref=(f"natural-message:{original_sha256}:metadata:{metadata_count:02d}"),
    ...
    for section in parsed.sections:
        ...
        refs.append(
            AdapterEvidenceRef(
                production_ref=(f"natural-message:{original_sha256}:{section.key}:{ordinal:02d}"),
...
def normalize_natural_packet_message(
    natural_message: str,
    *,
    benchmark_id: str,
    market: str,
) -> NaturalPacketAdapterResult:
    """Normalize KR/US production headings without changing message content."""
...
def validate_natural_packet_adapter_result(
    ...
    if _content_fingerprint(result.original_text) != _content_fingerprint(result.normalized_text):
        errors.append("natural_packet_content_mutated")
    ...
    if len({item.common_ref for item in result.evidence_ref_map}) != len(result.evidence_ref_map):
        errors.append("common_evidence_ref_collision")
