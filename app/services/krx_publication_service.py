...
class KrxProviderRoleEvidence(BaseModel):
    ...
    @model_validator(mode="after")
    def validate_observed_at(self) -> "KrxProviderRoleEvidence":
        if self.observed_at.tzinfo is None:
            raise ValueError("provider role evidence time must be timezone-aware")
        return self
...
def build_krx_publication_timeline(
    ...
    # An initial complete probe only proves an upper bound. A transition from a
    # tracked non-complete state identifies the first complete observation.
