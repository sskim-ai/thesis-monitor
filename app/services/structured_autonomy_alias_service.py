...
def _definition_key(schema_ref: str, prefix: str) -> str:
    expected = f"#/$defs/{prefix}"
    if not schema_ref.startswith(expected):
        raise ValueError(f"candidate_field_definition_scope_mismatch:{schema_ref}")
    key = schema_ref.removeprefix(expected)
    if not key:
        raise ValueError("candidate_field_definition_key_missing")
    return key


def _constrain_candidate_reference_fields(
    *,
    candidate_schema: dict[str, object],
    source_defs: dict[str, object],
    prefix: str,
    aliases_by_field: Mapping[str, Sequence[str]],
) -> dict[str, object]:
    properties = candidate_schema.get("properties")
    if not isinstance(properties, dict):
        raise ValueError("candidate_schema_properties_missing")

    alias_definitions: dict[str, object] = {}
    for field_name, aliases in aliases_by_field.items():
        property_schema = properties.get(field_name)
        if not isinstance(property_schema, dict):
            continue
        if not aliases or len(set(aliases)) != len(aliases):
            raise ValueError(f"invalid_field_alias_choices:{field_name}")

        safe_field = "".join(
            character if character.isalnum() else "_" for character in field_name
        )
        alias_definition = f"{prefix}EvidenceAlias_{safe_field}"
        ...
        schema_ref = direct_ref if isinstance(direct_ref, str) else item_ref
        if isinstance(schema_ref, str):
            definition_key = _definition_key(schema_ref, prefix)
            definition = source_defs.get(definition_key)
            if not isinstance(definition, dict):
                if property_schema.get("type") == "array":
                    property_schema["items"] = {
                        "$ref": f"#/$defs/{alias_definition}"
                    }
                    continue
                raise ValueError(
                    f"candidate_field_definition_missing:{field_name}:{definition_key}"
                )
            clone_key = f"{definition_key}__{safe_field}"
            clone = copy.deepcopy(definition)
            _constrain_reference_fields(clone, alias_definition)
            source_defs[clone_key] = clone
            constrained_ref = f"#/$defs/{prefix}{clone_key}"
            if isinstance(direct_ref, str):
                property_schema["$ref"] = constrained_ref
            else:
                property_schema["items"] = {"$ref": constrained_ref}
            continue

        if property_schema.get("type") == "array":
            property_schema["items"] = {"$ref": f"#/$defs/{alias_definition}"}
            continue
        _constrain_reference_fields(property_schema, alias_definition)
...
def build_alias_constrained_batch_schema(
    *,
    ...
    field_constraints = reference_aliases_by_ticker_field or {}
    unknown_tickers = set(field_constraints) - set(aliases_by_ticker)
    if unknown_tickers:
        raise ValueError(
            f"field_alias_constraint_ticker_scope_mismatch:{sorted(unknown_tickers)}"
        )
    for index, (ticker, aliases) in enumerate(aliases_by_ticker.items(), start=1):
        ...
        if not isinstance(source_defs, dict):
            raise ValueError("candidate_schema_definitions_invalid")
        scoped = _namespace_schema_refs(scoped, prefix)
        source_defs = _namespace_schema_refs(source_defs, prefix)
        if not isinstance(scoped, dict) or not isinstance(source_defs, dict):
            raise ValueError("candidate_schema_namespace_invalid")
        _constrain_reference_fields(scoped, alias_definition)
        _constrain_reference_fields(source_defs, alias_definition)
        ticker_field_constraints = field_constraints.get(ticker, {})
        all_aliases = set(aliases)
        for field_name, field_aliases in ticker_field_constraints.items():
            if not set(field_aliases).issubset(all_aliases):
                raise ValueError(
                    f"field_alias_constraint_expansion_forbidden:{ticker}:{field_name}"
                )
        scoped_alias_definitions = _constrain_candidate_reference_fields(
            candidate_schema=scoped,
            source_defs=source_defs,
            prefix=prefix,
            aliases_by_field=ticker_field_constraints,
        )
        ...
        root_defs.update(scoped_alias_definitions)
