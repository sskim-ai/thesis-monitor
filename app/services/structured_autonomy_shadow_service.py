...
def _packet_numeric_context(
    ...
    evidence = [
        ...
        for row in packet.evidence
        if not selected or row.ref_id in selected
    ]
    return {"evidence": evidence}


def _has_unowned_prose_number(
    text: str,
    packet: DecisionEvidencePacket,
    refs: Sequence[str] | None = None,
) -> bool:
    return bool(_prose_number_occurrences(text, _packet_numeric_context(packet, refs)))
...
def validate_structured_autonomy_candidate(
    ...
    *,
    ...
    if _has_unowned_prose_number(joined, packet):
