# 54 Program Completion

| Field | Value |
| --- | --- |
| architecture_semantic_drift | 0 |
| artifact_count | PENDING_FINALIZE |
| artifact_hash_mismatch_count | PENDING_FINALIZE |
| artifact_secret_scan_failure_count | PENDING_FINALIZE |
| artifact_size_mismatch_count | PENDING_FINALIZE |
| authorized_guard_compatibility_hash_drift | 1 |
| base_sha | eced7b9149e7d9ab7cbc5e3e3e6fab3fef1500d6 |
| batch_semantics | MODEL_CONTEXT_COUPLED |
| branch | codex/20260907-prespawn-live-workload-guard-compatibility-holdout-proof-resume |
| buy_without_nonprice_material_anchor | 0 |
| context_preservation_secondary_failure_count | 0 |
| continuation_adapter_semantic_mutation | 0 |
| contract | prespawn-live-workload-guard-compatibility-holdout-resume-v1 |
| current_holdout_reuse_allowed | 1 |
| diff_check | PENDING_FINAL_VALIDATION |
| directional_core_price_technical_refs | 0 |
| directional_core_supply_refs | 0 |
| directional_model_calls_on_source_insufficient | 0 |
| final_head_sha | 737bd21cf98d06bbaa00d77c53e928dae894b18e |
| first_hard_safety_gate_status | NOT_MEASURED |
| first_ownership_gate_status | NOT_MEASURED |
| first_renderer_gate_status | NOT_MEASURED |
| full_tests | PENDING_FINAL_VALIDATION |
| failure_class | RUNTIME_GENERATION_SCHEMA_CONST_MISMATCH |
| future_unseen_holdout_reuse_allowed | 0 |
| historical_stall_pattern_recurred | 0 |
| holdout_output_exposure_state | PARTIALLY_EXPOSED |
| holdout_retirement_state | RETIRED_PARTIAL_EXPOSURE |
| holdout_semantic_revelation_state | NOT_MEASURED_IDENTITY_GATE |
| implementation_commit | e78926cf246d6336889eab5373ae7423383bbf14 |
| instrumented_codex_lifecycle_mutation | 0 |
| latest_result_integrity | PASS |
| latest_result_zip_sha256 | fd519905d0201d5742d1c1b90c9b3e15b681020d05ad23a4031734a98eb73c68 |
| live_structured_autonomy_activation | 0 |
| live_v2_change | 0 |
| main_merge | 0 |
| model | gpt-5.6-sol |
| model_semantic_input_drift | 0 |
| model_timeout_owner_count | 1 |
| model_timeout_seconds | 1800 |
| monitoring_registration_calls | 0 |
| next_scope | BOUNDED_RUNTIME_IDENTITY_LOCK_REPAIR_NEW_HOLDOUT |
| night_futures_code_mutation | 0 |
| observation_unavailable_fail_closed_count | 0 |
| observation_unavailable_fail_open_count | 0 |
| ordered_cohort | ["NVDA", "JPM", "WMT", "BRK-B", "142210", "060900", "002680", "035420", "216050", "100700", "001530", "487580", "038870", "342870", "060230", "415380"] |
| original_root_exception | PermissionError:[Errno 1] Operation not permitted:ps |
| ownership_generalization_verdict | NOT_ESTABLISHED |
| ownership_proof_completion_state | INCOMPLETE_RUNTIME_IDENTITY_FAILURE |
| prespawn_guard_preflight_status | PASS |
| prespawn_guard_root_cause | LIVE_WORKLOAD_GUARD_PS_PERMISSION_DENIED |
| price_only_directional_model_calls | 0 |
| price_only_directional_ownership_violations | 0 |
| price_only_holder_reduce | 0 |
| price_timing_new_buyer_upgrade | 0 |
| production_db_mutation | 0 |
| production_scheduler_change | 0 |
| production_telegram_send | 0 |
| prompt_semantic_drift | 0 |
| protected_window_semantics_changed | 0 |
| ps_dependency_after_repair | False |
| readiness | NOT_READY |
| real_holdout_model_invocation_count | 1 |
| real_holdout_subject_output_count | 4 |
| reasoning_effort | xhigh |
| root_exception_masked_after_repair | 0 |
| root_exception_masked_before_repair | 1 |
| ruff | PENDING_FINAL_VALIDATION |
| run_a_hard_safety_gate_status | NOT_RUN |
| run_a_ownership_gate_status | NOT_RUN |
| run_a_renderer_gate_status | NOT_RUN |
| run_b_hard_safety_gate_status | NOT_RUN |
| run_b_ownership_gate_status | NOT_RUN |
| run_b_renderer_gate_status | NOT_RUN |
| run_c_hard_safety_gate_status | NOT_RUN |
| run_c_ownership_gate_status | NOT_RUN |
| run_c_renderer_gate_status | NOT_RUN |
| run_results | {"a": "NOT_RUN", "b": "NOT_RUN", "c": "NOT_RUN", "first": "FAILED_IDENTITY_GATE_0/16"} |
| safe_to_spawn | 1 |
| schema_semantic_drift | 0 |
| sell_without_nonprice_material_anchor | 0 |
| shared_context_subject_count | 4 |
| source_drift | 0 |
| source_generation_id | 20260907-us-remediation-holdout-20260907T001800Z-57bcd871e06a |
| source_lock | d4bd0b51d9cc543ebe03088047cce375db41d3d9a9f21c07d4fd5646bdf9b1ef |
| spawn_started_on_guard_failure | 0 |
| runtime_identity_expected | 20260907-prespawn-guard-resume-20260907T012700Z-1f19aa40aef6 |
| runtime_identity_received | 20260907-us-remediation-holdout-20260907T001800Z-57bcd871e06a |
| runtime_identity_schema_const_updated | 0 |
| stop_reason | RUNTIME_GENERATION_SCHEMA_CONST_MISMATCH:first:DIRECTIONAL_CORE:01 |
| supply_directional_core_usage | 0 |
| timeout_owner_mutation | 0 |
| timing_stage_balance_mutation | 0 |
| timing_stage_direction_mutation | 0 |
| timing_stage_hold_lean_mutation | 0 |
| transport_process_topology_mutation | 0 |
| transport_receipt_created_on_guard_failure | 0 |
| transport_receipt_expected_on_guard_failure | 0 |
| transport_retry_count | 0 |
| transport_timeout_count | 0 |
| work_instruction_commit | 78d05fea9690753095e42e403f3677b6901627bb |
| workload_observation_backend | LAUNCHCTL_JOB_STATE_PLUS_LSOF_CODEX_RUNTIME_STATE |
| workload_observation_capabilities | {"model_execution_observable": true, "model_execution_signal": "open /codex_runtime_state/ files", "natural_job_observable": true, "natural_job_signal": "loaded LaunchAgent state", "protected_window_observable": true, "ps_dependency": false} |

Machine proof: `proofs/54-program-completion.json`.