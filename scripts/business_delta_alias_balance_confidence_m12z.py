...
def business_delta_audit(
    ...
    *,
    ...
    view: BusinessDeltaEvidenceView | None = None,
    proof_critical: bool = False,
) -> dict[str, object]:
    if view is None and owned is not None:
    ...
    if view is not None:
        canonical = validate_business_delta_candidate(candidate, view)
        refs = canonical["selected_refs"]
        errors = list(canonical["errors"])
        if not refs:
            errors.append("BUSINESS_DELTA_EVIDENCE_REF_REQUIRED")
        errors = list(dict.fromkeys(errors))
        linked = canonical["linked_evidence"]
        return {
            **canonical,
            ...
            "errors": errors,
            "alias_resolution_failure_count": len(canonical["invalid_refs"]),
            "unsupported_absolute_state_to_delta_count": 0,
            "ineligible_selected_evidence_count": sum(
                not bool(row["delta_evidence_eligible"])
                for row in linked
            ),
            "semantic_source": "BusinessDeltaEvidenceView",
            "proof_critical_fallback_participation": False,
        }

    if proof_critical:
        return {
            "contract": "post-model-business-delta-validator-v2",
            "ticker": str(candidate.get("ticker") or ""),
            "status": "FAIL",
            "errors": ["CANONICAL_BUSINESS_DELTA_VIEW_REQUIRED"],
            "semantic_source": "CANONICAL_VIEW_REQUIRED",
            "proof_critical_fallback_participation": False,
    ...
    return {
        ...
        "errors": errors,
        "semantic_source": "NON_PROOF_LEGACY_COMPATIBILITY",
        "proof_critical_fallback_participation": False,
    }
