...
from app.services.market_expectation_evidence_service import (
    ...
    constrain_market_expectation_batch_schema,
...
from scripts.shadow_frozen_context_manifest import verify_frozen_context_inputs
from scripts.context_preserving_finalization import finalize_fictional_contexts
from scripts.finalization_readiness_policy import evaluate_hard_readiness
...
FROZEN_PACKET_SOURCE_OVERRIDE: Path | None = None
PREVIOUS_BUNDLE = (
...
# Frozen historical cohort metadata; current shadow gates derive counts from state.
EXPECTED_SHADOW_MODEL_CALLS = 18

_FICTIONAL_EXPECTATION_DEPENDENCY_LABELS = {
    "FIC-FIN-05": ("market-expectation", "remaining-unknown"),
}
...
def _expectation_views(
    ...
    catalogs: Mapping[str, EvidenceAliasCatalog],
    *,
    structured_bases: Mapping[
        str, Sequence[MarketExpectationStructuredBasis]
    ] | None = None,
) -> dict[str, MarketExpectationEvidenceView]:
    bases = structured_bases or {}
    return {
        ticker: build_market_expectation_evidence_view(
            item,
            catalogs[ticker],
            structured_bases=bases.get(ticker, ()),
        )
        for ticker, item in owned.items()
    }


def _fictional_expectation_structured_bases(
    catalogs: Mapping[str, EvidenceAliasCatalog],
) -> dict[str, tuple[MarketExpectationStructuredBasis, ...]]:
    """Return fixture-authored dependencies without deriving them from prose."""

    result: dict[str, tuple[MarketExpectationStructuredBasis, ...]] = {
        ticker: () for ticker in catalogs
    }
    for ticker, labels in _FICTIONAL_EXPECTATION_DEPENDENCY_LABELS.items():
        catalog = catalogs.get(ticker)
        if catalog is None:
            continue
        by_label = {entry.label: entry.canonical_ref for entry in catalog.entries}
        expectation_label, dependency_label = labels
        if expectation_label not in by_label or dependency_label not in by_label:
            raise ValueError(
                f"fictional_expectation_dependency_fixture_missing:{ticker}"
            )
        result[ticker] = (
            MarketExpectationStructuredBasis(
                expectation_ref=by_label[expectation_label],
                dependency_refs=(by_label[dependency_label],),
            ),
        )
    return result


def _expectation_view_manifest(
    views: Mapping[str, MarketExpectationEvidenceView],
) -> dict[str, object]:
    rows = []
    for ticker, view in views.items():
        expectations = view.expectation_items
        rows.append(
            {
                "ticker": ticker,
    ...
    return {
        "status": "PASS",
        "contract": "market-expectation-evidence-view-v1",
        "subject_count": len(rows),
        "expectation_ref_count": sum(
            int(row["expectation_ref_count"]) for row in rows
        ),
        "independent_directional_support_count": sum(
            int(row["independent_directional_support_count"]) for row in rows
        ),
        "conditional_context_only_count": sum(
            int(row["conditional_context_only_count"]) for row in rows
        ),
        "independence_unknown_count": sum(
            int(row["independence_unknown_count"]) for row in rows
        ),
        "already_reflected_counterweight_count": sum(
            int(row["already_reflected_counterweight_count"]) for row in rows
        ),
        "rows": rows,
    }
...
def _enriched_contexts(
    ...
    views: Mapping[str, BusinessDeltaEvidenceView],
    *,
    expectation_views: Mapping[str, MarketExpectationEvidenceView] | None = None,
) -> dict[str, dict[str, object]]:
    enriched = {
        ticker: attach_business_delta_evidence_view(context, views[ticker])
        for ticker, context in contexts.items()
    }
    if expectation_views is None:
        return enriched
    if set(expectation_views) != set(contexts):
        raise ValueError("MARKET_EXPECTATION_VIEW_CONTEXT_SCOPE_MISMATCH")
    return {
        ticker: attach_market_expectation_evidence_view(
            enriched[ticker],
            expectation_views[ticker],
        )
    ...
    *,
    ...
    if views is None:
        schema = build_alias_constrained_batch_schema(
            ...
            reference_aliases_by_ticker_field=configured_field_constraints,
        )
    else:
    ...
    if expectation_views is None:
        return schema
    return constrain_market_expectation_batch_schema(
        schema,
        views={ticker: expectation_views[ticker] for ticker in tickers},
    )
...
def _expectation_audit_rows(
    ...
    for row in rows:
        ticker = str(row["ticker"])
        expectation = validate_market_expectation_candidate(
            row["core"],
            views[ticker],
        ...
        if expectation["status"] != "PASS":
            errors.extend(str(error) for error in expectation["errors"])
            violations += 1
        row["market_expectation_independence"] = expectation
        row["errors"] = list(dict.fromkeys(errors))
        row["status"] = "PASS" if not row["errors"] else "FAIL"
    return rows, violations
...
def _stage1_audit(
    ...
    *,
    ...
    if expectation_views is not None:
        rows, expectation_violations = _expectation_audit_rows(
            rows,
            expectation_views,
        )
    return rows, {
        **audit,
        ...
        "market_expectation_independence_violation_count": expectation_violations,
...
def _previous_source_packets() -> tuple[dict[str, object], dict[str, dict[str, object]]]:
    source_output = FROZEN_PACKET_SOURCE_OVERRIDE or PREVIOUS_OUTPUT
    state = read_json(source_output / "shadow/program-state.json")
    ...
    *,
    ...
    gate_pass = all(
        (
            ...
            ruff["status"] == "PASS",
            diff["status"] == "PASS",
            len(packets) == len(previous_state["tickers"]),
...
def _freeze_fictional(preflight: Mapping[str, object]) -> dict[str, object]:
    ...
    expectation_views = _expectation_views(
        owned,
        catalogs,
        structured_bases=_fictional_expectation_structured_bases(catalogs),
    )
    expectation_manifest = _expectation_view_manifest(expectation_views)
    if manifest["status"] != "PASS":
        raise ValueError("FICTIONAL_DELTA_CAPABILITY_INPUT_AMBIGUOUS")
    enriched = _enriched_contexts(
        contexts,
        views,
        ...
        frozen_rows.append(
            {
                ...
                "market_expectation_view_sha256": canonical_sha256(
                    {
                        ticker: expectation_views[ticker].model_context()
                        for ticker in tickers
                    }
                ),
            }
    ...
    state = {
        ...
        "expectation_views": {
            ticker: view.model_dump(mode="json")
    ...
    gate = {
        **dict(preflight),
        "status": (
            ...
            and expectation_manifest["status"] == "PASS"
            and manifest["subject_count"] == 8
            and expectation_manifest["subject_count"] == 8
            else "FAIL"
        ),
        "fictional_capability_manifest": manifest["status"],
        "fictional_expectation_manifest": expectation_manifest["status"],
        "planned_model_calls": FICTIONAL_MODEL_CALLS,
        "model_calls_before_gate": 0,
    }
...
def _restore_expectation_views(
    state: Mapping[str, object],
) -> dict[str, MarketExpectationEvidenceView] | None:
    rows = state.get("expectation_views")
    if rows is None:
        return None
    if not isinstance(rows, Mapping):
        raise ValueError("MARKET_EXPECTATION_VIEWS_INVALID")
    return {
        str(ticker): MarketExpectationEvidenceView.model_validate(value)
        for ticker, value in rows.items()
    }
...
def run_fictional() -> None:
    ...
    if {
        ticker: view.model_dump(mode="json")
        for ticker, view in expectation_views.items()
    } != state.get("expectation_views"):
        raise ValueError("FICTIONAL_MARKET_EXPECTATION_VIEW_DRIFT")
...
def finalize_fictional() -> None:
    ...
    packets, owned, catalogs, contexts = m12.fictional_inputs(generation_id)
    views = _views(owned, catalogs, contexts)
    expectation_views = _restore_expectation_views(state)
    if expectation_views is None:
        raise ValueError("FICTIONAL_MARKET_EXPECTATION_VIEWS_MISSING")
    expected_membership = {
    ...
    variance_diagnostics = {
        "primary_direction_unstable_subject_count": sum(
            row["classification"] == "VARIABLE" for row in direction
        ),
        "business_delta_unstable_subject_count": sum(
            row["classification"] == "VARIABLE" for row in business
        ),
        "new_buyer_unstable_subject_count": sum(
            row["classification"] == "VARIABLE" for row in buyer
        ),
        "holder_unstable_subject_count": sum(
            row["classification"] == "VARIABLE" for row in holder
        ),
    }
    readiness = evaluate_hard_readiness(
        hard_gates={
            "stage1_row_count": len(stage1_rows) == FICTIONAL_OUTPUT_COUNT,
            "stage2_row_count": len(stage2_rows) == FICTIONAL_OUTPUT_COUNT,
            "final_composition_count": len(final_rows) == FICTIONAL_OUTPUT_COUNT,
            "context_boundary_integrity": finalization["status"] == "PASS",
            "unique_final_identity": (
                finalization["unique_identity_count"] == FICTIONAL_OUTPUT_COUNT
            ),
            "stage1_semantics": stage1_errors == 0,
            "stage2_semantics": stage2_errors == 0,
            "final_semantics": hard_errors == 0,
            "business_delta_capability": capability_violations == 0,
            "core_immutability": mutation_count == 0,
            "runtime": runtime_audit["status"] == "PASS",
        },
        variance_diagnostics=variance_diagnostics,
    )
    hard_pass = readiness["status"] == "PASS"
    decision = {
        ...
        "context_count": finalization["context_count"],
        "directional_core_batch_over_limit_use_count": finalization[
            "directional_core_batch_over_limit_use_count"
        ],
        "context_boundary_integrity": finalization,
        "business_delta_capability_violation_count": capability_violations,
        "core_mutation_count": mutation_count,
        **variance_diagnostics,
        "hard_readiness": readiness,
...
def prepare_shadow() -> None:
    ...
    expectation_views = _expectation_views(owned, catalogs)
    expectation_manifest = _expectation_view_manifest(expectation_views)
    ...
    gate_pass = all(
        (
            ...
            len(tickers) == len(previous_tickers),
            len(context_rows) == len(_batches(tickers)),
            len(context_rows) * 3 == state["planned_model_calls"],
            int(schedule["observed_paused_schedule_count"]) >= 4,
...
def run_shadow() -> None:
    ...
    if {
        ...
        raise ValueError("SHADOW_MARKET_EXPECTATION_VIEW_DRIFT")
    generation_id = str(state["generation_id"])
    ...
    try:
        for context_number, batch_tickers in enumerate(_batches(tickers), start=1):
            ...
            print(
                f"M12AI_SHADOW_START {completed + 1}/{planned_model_calls} "
                f"{invocation_id}",
                flush=True,
            )
            receipt = base._checked_model_call(
            ...
            print(
                f"M12AI_SHADOW_COMPLETE {completed}/{planned_model_calls} PASS",
                flush=True,
            )
        ...
        raise
    if completed != planned_model_calls:
...
def finalize_shadow() -> None:
    ...
    if complete.get("model_calls") != planned_model_calls:
    ...
    if not all(
        len(documents) == expected_context_count
        for documents in (
    ...
    runtime_audit = {
        "status": (
            "PASS"
            if len(runtime) == planned_model_calls
            and all(row["status"] == "PASS" for row in runtime)
            and all(
                row.get("observed_runtime") == {"model": MODEL, "effort": EFFORT}
                for row in runtime
