...
CONTRACT = "kr-us-monitoring-infra-kr-live-e2e-v1"
...
def _reset_isolated_deliveries(run_date: date, *, tickers: set[str]) -> int:
    reset = 0
...
def _delivery_audit(run_date: date, *, tickers: set[str]) -> dict[str, int]:
    with Session(engine) as session:
        rows = session.exec(
            select(NotificationDelivery).where(
                NotificationDelivery.assessment_date == run_date,
...
def _prepare_isolated_runtime(args: argparse.Namespace) -> tuple[dict[str, object], Path]:
    ...
    if not packet_id or packet.get("market") != "kr" or len(packet.get("stocks") or []) != 8:
...
async def _run(args: argparse.Namespace) -> dict[str, object]:
    ...
    if model_receipt.get("status") != "PASS" or model_receipt.get("ready_count") != 8:
        raise ValueError(f"live_e2e_accepted_v2_not_ready:{model_receipt.get('status')}")
    if int(model_receipt.get("claim_lease_renewal_count") or 0) < 1:
        raise ValueError("live_e2e_claim_heartbeat_not_observed")

    backup = claim_next_ai_review_packet(
        "kr",
        owner="kr-us-infra-live-e2e-backup",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run isolated integrated KR monitoring E2E")
