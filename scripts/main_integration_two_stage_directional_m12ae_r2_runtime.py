...
STAGE2_LANGUAGE_CONTAMINATION_CONTRACT = "stage2-language-contamination-v2"
...
# Each rule preserves an existing forbidden lexeme while avoiding raw substring
# matches inside unrelated Korean business words such as `수주가` and `발주가`.
STAGE2_LANGUAGE_CONTAMINATION_RULES = (
    (
        "latin-rsi-token",
        "rsi",
        "LEXICALLY_UNAMBIGUOUS_WITH_TOKEN_BOUNDARY",
        re.compile(r"(?<![a-z0-9])rsi(?![a-z0-9])", re.IGNORECASE),
    ),
    (
        "latin-macd-token",
        "macd",
        "LEXICALLY_UNAMBIGUOUS_WITH_TOKEN_BOUNDARY",
        re.compile(r"(?<![a-z0-9])macd(?![a-z0-9])", re.IGNORECASE),
    ),
    (
        "latin-bollinger-token",
        "bollinger",
        "LEXICALLY_UNAMBIGUOUS_WITH_TOKEN_BOUNDARY",
        re.compile(r"(?<![a-z0-9])bollinger(?![a-z0-9])", re.IGNORECASE),
    ),
    (
        "english-support-level-phrase",
        "support level",
        "PHRASE_LEVEL_CONTAMINATION_PATTERN",
        re.compile(
            r"(?<![a-z0-9])support\s+level(?![a-z0-9])", re.IGNORECASE
        ),
    ),
    (
        "english-resistance-level-phrase",
        "resistance level",
        "PHRASE_LEVEL_CONTAMINATION_PATTERN",
        re.compile(
            r"(?<![a-z0-9])resistance\s+level(?![a-z0-9])", re.IGNORECASE
        ),
    ),
    (
        "korean-share-price-left-boundary",
        "주가",
        "AMBIGUOUS_KOREAN_LEXEME_WITH_LEFT_BOUNDARY",
        re.compile(
            r"(?<![가-힣a-z0-9])(?:(?:현재|당일)주가|주가)", re.IGNORECASE
        ),
    ),
    (
        "korean-chart-left-boundary",
        "차트",
        "LEXICALLY_UNAMBIGUOUS_WITH_LEFT_BOUNDARY",
        re.compile(r"(?<![가-힣a-z0-9])차트", re.IGNORECASE),
    ),
    (
        "korean-technical-timing-phrase",
        "기술적",
        "AMBIGUOUS_KOREAN_LEXEME_REQUIRING_TIMING_PHRASE",
        re.compile(
            r"(?<![가-힣a-z0-9])기술적(?:인)?\s*"
            r"(?:지지|저항|반등|돌파|차트|신호|모멘텀|매수|매도|이탈|"
            r"진입(?!장벽))",
            re.IGNORECASE,
        ),
    ),
    (
        "korean-market-flow-context",
        "수급",
        "AMBIGUOUS_KOREAN_LEXEME_REQUIRING_MARKET_CONTEXT",
        re.compile(
            r"(?<![가-힣a-z0-9])(?:"
            r"(?:외국인|기관|개인|시장)\s*(?:의\s*)?수급|"
            r"수급[^.!?\n]{0,40}?"
            r"(?:진입(?!장벽)|매수|매도|보유|비중|주가|차트|지지|저항|"
            r"돌파|반등)"
            r")",
            re.IGNORECASE,
        ),
    ...
    (
        "korean-trading-volume-left-boundary",
        "거래량",
        "LEXICALLY_UNAMBIGUOUS_WITH_LEFT_BOUNDARY",
        re.compile(r"(?<![가-힣a-z0-9])거래량", re.IGNORECASE),
    ),
)
    ...
    *,
    ...
    for row in rows:
        ...
        delta = business_delta.business_delta_audit(
            ...
            catalogs[ticker],
            owned=owned[ticker],
            view=views[ticker] if views is not None else None,
            proof_critical=True,
        )
...
def _stage2_text_fields(
    candidate: FundamentalStanceCandidate,
) -> tuple[tuple[str, str], ...]:
    buyer = candidate.fundamental_new_buyer
    holder = candidate.fundamental_holder
    return (
        ("fundamental_new_buyer.summary", buyer.summary),
        (
            "fundamental_new_buyer.confirmation_business_condition",
            buyer.confirmation_business_condition,
        ),
        ("fundamental_holder.summary", holder.summary),
        (
            "fundamental_holder.business_invalidation_condition",
...
def _stage2_texts(candidate: FundamentalStanceCandidate) -> tuple[str, ...]:
    return tuple(text for _path, text in _stage2_text_fields(candidate))


def stage2_language_contamination_spans(
    text: str,
    *,
    text_path: str = "text",
) -> tuple[dict[str, object], ...]:
    spans: list[dict[str, object]] = []
    for rule_id, lexeme, risk_class, pattern in STAGE2_LANGUAGE_CONTAMINATION_RULES:
        for match in pattern.finditer(text):
            spans.append(
                {
                    "text_path": text_path,
                    "matched_contamination_span": match.group(0),
    ...
    return tuple(
        sorted(
            ...
            key=lambda row: (
                ...
                int(row["match_start"]),
                int(row["match_end"]),
                str(row["match_rule_id"]),
            ),
        )
    )


def _stage2_language_contamination_spans(
    candidate: FundamentalStanceCandidate,
) -> tuple[dict[str, object], ...]:
    return tuple(
        span
        for path, text in _stage2_text_fields(candidate)
        for span in stage2_language_contamination_spans(text, text_path=path)
    )
...
def _stage2_audit(
    ...
    *,
    ...
    for candidate in batch.candidates:
        ...
        matched_spans = list(_stage2_language_contamination_spans(candidate))
        language_contamination = sorted(
            {str(span["matched_lexeme"]) for span in matched_spans}
        )
        errors = []
        if invalid_refs:
        ...
        rows.append(
            {
                ...
                "scanned_text_paths": list(STAGE2_SCANNED_TEXT_PATHS),
                "language_contamination": language_contamination,
                "matched_contamination_spans": matched_spans,
            }
        )
    timing_ref_count = sum(len(row["timing_or_supply_refs"]) for row in rows)
    language_span_count = sum(len(row["matched_contamination_spans"]) for row in rows)
    return rows, {
        "contract": STAGE2_LANGUAGE_CONTAMINATION_CONTRACT,
        "row_count": len(rows),
        "pass_count": sum(row["status"] == "PASS" for row in rows),
        ...
        "timing_or_supply_ref_contamination_row_count": sum(
            bool(row["timing_or_supply_refs"]) for row in rows
        ),
        "language_contamination_count": language_span_count,
        "language_contamination_row_count": sum(
            bool(row["matched_contamination_spans"]) for row in rows
        ),
...
def finalize_fictional() -> None:
    ...
    def finalize_context(
        ...
        if audit["status"] != "PASS":
            raise ValueError(
                "CONTEXT_FINAL_CANDIDATE_AUDIT_FAILED:"
                f"{stage2['repetition']}:{stage2['context']}"
            )
        return rows
    ...
    final_audit_rows = finalization["final_rows"]
    final_audit_errors = sum(len(row["errors"]) for row in final_audit_rows)
    ...
    readiness_result = evaluate_hard_readiness(
        hard_gates={
            "stage1_row_count": len(stage1_rows) == FICTIONAL_FINAL_ROWS,
            "stage2_row_count": len(stage2_rows) == FICTIONAL_FINAL_ROWS,
            "final_composition_count": len(final_candidates)
            == FICTIONAL_FINAL_ROWS,
            "context_boundary_integrity": finalization["status"] == "PASS",
            "unique_final_identity": finalization["unique_identity_count"]
            == FICTIONAL_FINAL_ROWS,
            "stage1_semantics": stage1_errors == 0,
            "stage2_semantics": stage2_errors == 0,
            "final_semantics": final_audit_errors == 0,
            "core_immutability": mutation_count == 0,
            "runtime_call_count": runtime_counts["model_calls"]
            == FICTIONAL_STAGE_CALLS,
            "runtime_pass_count": runtime_counts["pass_count"]
            == FICTIONAL_STAGE_CALLS,
            "runtime_timeout": runtime_counts["timeout_count"] == 0,
            "runtime_capacity": runtime_counts["capacity_failure_count"] == 0,
            "runtime_orphan": runtime_counts["orphan_process_count"] == 0,
            "runtime_retry": runtime_counts["wrapper_retry_count"] == 0,
            "runtime_model_target": runtime_counts["runner_model_target_match"],
        },
        variance_diagnostics=variance_diagnostics,
    )
    readiness = readiness_result["status"] == "PASS"
    ...
    report(52, "full-stage1-core-stability", {"status": "MEASURED", "readiness_blocking": False, "unstable_subject_count": direction_unstable, "rows": direction})
    ...
    report(54, "full-stage2-new-buyer-stability", {"status": "MEASURED", "readiness_blocking": False, "rows": buyer, "unstable_subject_count": buyer_unstable})
    report(55, "full-stage2-holder-stability", {"status": "MEASURED", "readiness_blocking": False, "rows": holder, "unstable_subject_count": holder_unstable, "fic_fin_05_holder_values": holder_fic05})
    ...
    report(58, "full-final-decision-material-stability", {"status": "MEASURED", "readiness_blocking": False, "primary_direction": direction, "business_delta": business, "new_buyer": buyer, "holder": holder})
    ...
    report(61, "full-business-delta-audit", {"status": "PASS" if all(row["business_delta"]["status"] == "PASS" for row in final_audit_rows) else "FAIL", "readiness_blocking_variance": False, "unstable_subject_count": business_unstable, "rows": business})
    ...
    decision = {
        ...
        "hard_readiness": readiness_result,
