...
def _augment_run(
    ...
    semantic_audits = read_json(
        runner.proof_path(args.report_dir, runner.RUN_PROOFS[run][2])
    )
    ...
    core_audits = [
        row
        for row in semantic_audits.get("rows", ())
        if isinstance(row, Mapping) and row.get("stage") == "DIRECTIONAL_CORE"
    ]
    canonical_semantic_provenance = bool(core_audits) and all(
        row.get("status") == "PASS"
        and row.get("canonical_semantic_audit_consumed") is True
        for row in core_audits
    )
    document.update(
        {
            ...
            "canonical_semantic_audit_provenance_status": (
                "PASS" if canonical_semantic_provenance else "FAIL"
            ),
