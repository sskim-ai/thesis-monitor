"""M12BE report-count compatibility repair and full monitored shadow proof."""
...
SOURCE_OUTPUT = OUTPUT / "frozen-m12bd-shadow-source"
RUNNER = Path("scripts/shadow_active_universe_count_compatibility_m12be.py")
ARCHITECTURE = Path(
    "docs/architecture/SHADOW_ACTIVE_UNIVERSE_COUNT_CONTRACT_COMPATIBILITY.md"
)
WORK_INSTRUCTION = Path("docs/work-instructions") / f"{NAME}.md"
WORK_INSTRUCTION_COMMIT = "e939ffd1e8e8e0ff68959baed0976fe5c7dfd0ef"
BASE_INTEGRATION_HEAD_SHA = "255cdb081c55b4f6f30724231c318ce591ec2fd8"
...
M12BD_STOPPED_SHADOW_GENERATION_ID = (
    "20260911-m12ai-shadow-20260913T121804Z-145705ea053d"
)
...
shadow-preflight-active-count-failure-reproduction
expectation-manifest-contract-audit
active-count-consumer-code-audit
active-count-hardcode-repository-scan
shadow-count-contract-options
shadow-count-contract-decision
contract-aware-subject-count-adapter-contract
active-universe-ticker-set-identity-contract
cross-manifest-shadow-preflight-contract
...
m12bd-stopped-shadow-preflight-offline-replay
...
count-contract-unit-tests
shadow-preflight-ticker-set-tests
stopped-shadow-offline-preflight-replay
...
shadow-count-contract-repair-success-decision
...
def _verify_latest_bundle() -> dict[str, object]:
    actual_sha = file_sha256(M12BD_BUNDLE)
    hash_mismatches = 0
    size_mismatches = 0
    with zipfile.ZipFile(M12BD_BUNDLE) as archive:
        index_path = f"{M12BD_ARTIFACT_ROOT}/artifact-index.json"
        ...
        entry_count = len([name for name in archive.namelist() if not name.endswith("/")])
    status = all(
        (
            actual_sha == M12BD_BUNDLE_SHA256,
            index.get("status") == "PASS",
            index.get("artifact_count") == len(index["rows"]),
            hash_mismatches == 0,
            size_mismatches == 0,
            entry_count == len(index["rows"]) + 1,
        )
    )
    return {
        "status": "PASS" if status else "FAIL",
        "path": str(M12BD_BUNDLE),
        "expected_sha256": M12BD_BUNDLE_SHA256,
...
def _extract_reusable_proof_and_packets() -> dict[str, object]:
    proof_prefix = f"{M12BD_ARTIFACT_ROOT}/proof-runtime/"
    ...
    if canonical_mismatches:
        raise ValueError(
            f"M12BD_FROZEN_PACKET_HASH_MISMATCH:{canonical_mismatches}"
        )
    return {
        "status": "PASS",
    ...
    with zipfile.ZipFile(M12BD_BUNDLE) as archive:
        return {
            "configured_signal": _zip_read_json(
                archive, _m12bd_proof_report_path(archive, "m12ba", 98)
            ),
            "configured_financial_support": _zip_read_json(
                archive, _m12bd_proof_report_path(archive, "m12ba", 99)
            ),
            "business_delta": _zip_read_json(
                archive, _m12bd_proof_report_path(archive, "m12ba", 100)
            ),
            "market_expectation": _zip_read_json(
                archive, _m12bd_proof_report_path(archive, "m12ba", 101)
            ),
            "frozen_context": _zip_read_json(
                archive, _m12bd_proof_report_path(archive, "m12ba", 102)
            ),
            "batching": _zip_read_json(
                archive, _m12bd_proof_report_path(archive, "m12ba", 103)
            ),
        }


def _offline_stopped_shadow_replay() -> dict[str, object]:
    reports = _old_shadow_reports()
    source_state = read_json(SOURCE_OUTPUT / "shadow/program-state.json")
    active_tickers = _strict_unique_tickers(
        [row["ticker"] for row in source_state["universe"]],
        subject="m12bd_stopped_active_universe",
    )
    expectation = report_subject_identity(reports["market_expectation"])
    _configure_proof_runtime()
    views, _catalogs, _contexts = m12bd.m12bb._shadow_views(source_state)
    inventories = {
        "packet_inventory": tuple(source_state["packet_hashes"]),
        "configured_signal_view": _report_tickers(reports["configured_signal"]),
        "configured_financial_support": _report_tickers(
            reports["configured_financial_support"]
        ),
        "business_delta_view": _report_tickers(reports["business_delta"]),
        "market_expectation_view": expectation.tickers,
        "stage1_wc_binding_view": tuple(views),
        "stage2_wc_binding_view": tuple(views),
        "frozen_context_manifest": _report_tickers(reports["frozen_context"]),
        "batching_manifest": _report_tickers(reports["batching"]),
    }
    identity = audit_subject_set_identity(active_tickers, inventories)
    return {
        **identity,
        "status": "PASS" if identity["status"] == "PASS" else "FAIL",
        "root_cause_repaired": "SHADOW_ACTIVE_UNIVERSE_REPORT_COUNT_FIELD_NAME_DRIFT",
        "active_universe_count": len(active_tickers),
        "expectation_manifest_contract": expectation.contract,
        "expectation_manifest_count_field": expectation.count_field,
        "expectation_subject_count": expectation.reported_count,
        "expectation_row_count": expectation.actual_row_count,
        "packet_count": len(source_state["packet_hashes"]),
        "old_shadow_generation_id": source_state["generation_id"],
...
def prepare() -> None:
    if OUTPUT.exists() or REPORTS.exists():
        raise ValueError("M12BE_GENERATION_ALREADY_PREPARED")
    if git("status", "--porcelain", "--untracked-files=no"):
        raise ValueError("M12BE_PREPARE_REQUIRES_COMMITTED_CODE")
    integrity = _verify_latest_bundle()
    if integrity["status"] != "PASS":
        raise SystemExit("M12BD_RESULT_BUNDLE_INTEGRITY_FAILURE")
    ...
    source_identity = audit_subject_set_identity(
        active_tickers,
        {"m12bd_frozen_packet_source": tuple(stopped_state["packet_hashes"])},
    )
    reports = _old_shadow_reports()
    expectation = report_subject_identity(reports["market_expectation"])
    offline = _offline_stopped_shadow_replay()
    fixtures = _fixture_results(active_tickers)
    ...
    gate_pass = all(
        (
            ...
            offline["status"] == "PASS",
            all(item["status"] == "PASS" for item in fixtures.values()),
            len(before_scan) == 1,
            len(after_scan) == 0,
            tests["status"] == "PASS",
    ...
    report(
        "m12be-scope-freeze",
        {
            "status": "FROZEN",
            "phase": "M12BE",
            "root_cause": "SHADOW_ACTIVE_UNIVERSE_REPORT_COUNT_FIELD_NAME_DRIFT",
            "model": MODEL,
            "reasoning_effort": EFFORT,
            "new_formal_model_calls": 0,
    ...
    report(
        ...
        {
            "status": "PASS",
            "consumer": "scripts/financial_sector_replacement_verb_parity_m12ba.py:prepare_shadow",
            "stale_mapping_key": "active_count",
            "authoritative_field": expectation.count_field,
            "observed_exception": "KeyError: active_count",
    ...
    report(
        "expectation-manifest-contract-audit",
        {
            ...
            "authoritative_count_field": expectation.count_field,
            "reported_count": expectation.reported_count,
            "row_count": expectation.actual_row_count,
            "ticker_set_sha256": expectation.ticker_set_sha256,
            "producer_contract_changed": False,
        },
    )
    report(
        "active-count-consumer-code-audit",
        {
            "status": "PASS" if len(before_scan) == 1 and not after_scan else "FAIL",
            "before_count": len(before_scan),
            "after_count": len(after_scan),
            "before": before_scan,
            "after": after_scan,
        },
    )
    report(
        "active-count-hardcode-repository-scan",
        {
            "status": "PASS" if not after_scan else "FAIL",
            "scope": "DIRECT_MAPPING_ACCESS_TO_ACTIVE_COUNT_IN_PYTHON",
            "before_count": len(before_scan),
            "after_count": len(after_scan),
            "matches": after_scan,
        },
    ...
    report(
        "shadow-count-contract-options",
        {
            ...
            "options": [
                {
                    "option": "duplicate_active_count_in_producer",
                    "decision": "REJECTED",
                    "reason": "POLLUTES_VERSIONED_PRODUCER_CONTRACT",
                },
                {
                    "option": "guess_first_count_like_field",
                    "decision": "REJECTED",
                    "reason": "UNKNOWN_CONTRACT_MUST_FAIL_CLOSED",
                },
                {
                    "option": "contract_aware_consumer_adapter",
                    "decision": "SELECTED",
                    "reason": "PRESERVES_AUTHORITATIVE_CONTRACT",
                },
            ],
        },
    )
    report(
        ...
        {
            "status": "SELECTED",
            "decision": "CONTRACT_AWARE_SUBJECT_COUNT_PLUS_EXACT_TICKER_SET",
            "producer_change_count": 0,
            "hard_coded_active_count": False,
        },
    )
    report(
        "contract-aware-subject-count-adapter-contract",
        {
            "status": "PASS",
            "contract": COUNT_ADAPTER_CONTRACT,
            "recognized_report_contracts": {
                MARKET_EXPECTATION_EVIDENCE_VIEW_CONTRACT: "subject_count"
            },
    ...
    report(
        "active-universe-ticker-set-identity-contract",
        {
            "status": "PASS",
            "contract": COUNT_ADAPTER_CONTRACT,
            "count_only_is_sufficient": False,
            "exact_ticker_set_required": True,
            "duplicate_ticker_allowed": False,
            "missing_or_extra_ticker_allowed": False,
        },
    )
    report(
        "cross-manifest-shadow-preflight-contract",
        {
            "status": "PASS",
            "required_sources": [
                "task_start_active_universe",
                "packet_inventory",
                "configured_signal_view",
                "configured_financial_support",
                "business_delta_view",
                "market_expectation_view",
                "stage1_wc_binding_view",
                "stage2_wc_binding_view",
                "frozen_context_manifest",
                "batching_manifest",
            ],
            "count_and_exact_ticker_set_required": True,
        },
    )
    for slug, value in fixtures.items():
        report(slug, value)
    report("m12bd-stopped-shadow-preflight-offline-replay", offline)
    ...
    freeze_mapping = {
        ...
        "m12ax-monitoring-semantics-freeze": 41,
        "m12aw-nominal-condition-freeze": 42,
        "m12av-mixed-risk-scope-freeze": 43,
    ...
    for slug, number in freeze_mapping.items():
        report(slug, frozen_reports[number])
    ...
    semantic_mapping = {
        ...
        "expectation-view-semantic-hash-freeze": "expectation_view",
        "financial-evidence-projection-hash-freeze": "financial_evidence_projection",
        "final-user-schema-hash-freeze": "final_user_schema",
    }
    ...
    report("count-contract-unit-tests", tests["focused"])
    report(
        "shadow-preflight-ticker-set-tests",
        {
            "status": (
                "PASS"
                if all(value["status"] == "PASS" for value in fixtures.values())
                else "FAIL"
            ),
            "fixtures": fixtures,
        },
    )
    report("stopped-shadow-offline-preflight-replay", offline)
    ...
    state = {
        ...
        "formal_fictional_reuse_status": reuse_status,
        "new_fictional_model_calls": 0,
        "old_shadow_generation_id": stopped_state["generation_id"],
    ...
    if not gate_pass:
        raise SystemExit("M12BE_NEW_SHADOW_MODEL_CALL_GATE_FAILED")
...
def prepare_shadow() -> None:
    ...
    if program.get("status") != "FROZEN":
        raise ValueError("M12BE_PREMODEL_GATE_REQUIRED")
    if program.get("implementation_head_sha") != git("rev-parse", "HEAD"):
        raise ValueError("M12BE_CODE_CHANGED_AFTER_PREMODEL_FREEZE")
    if program.get("formal_fictional_reuse_status") != (
        "REUSE_AUTHORIZED_M12BD_COMPLETE_PROOF"
    ):
        raise ValueError("M12BE_FORMAL_REUSE_NOT_AUTHORIZED")

    _configure_proof_runtime()
    m12bd.m12bb.prepare_shadow()
    state = read_json(PROOF_OUTPUT / "shadow/program-state.json")
    if state["generation_id"] == M12BD_STOPPED_SHADOW_GENERATION_ID:
        raise ValueError("M12BE_STOPPED_SHADOW_GENERATION_REUSE_FORBIDDEN")
    ...
    if tuple(program["active_tickers"]) != active_tickers:
        raise ValueError("M12BE_TASK_START_ACTIVE_UNIVERSE_DRIFT")
    ...
    report(
        "new-shadow-generation-manifest",
        {
            ...
            "prior_stopped_generation_id": M12BD_STOPPED_SHADOW_GENERATION_ID,
    ...
    report(
        "shadow-packet-inventory",
        {
            ...
            "source_generation_id": M12BD_STOPPED_SHADOW_GENERATION_ID,
    ...
    if not gate_pass:
        raise SystemExit("M12BE_NEW_SHADOW_PREFLIGHT_FAILED")
...
def run_shadow() -> None:
    ...
    if state.get("status") != "FROZEN":
        raise ValueError("M12BE_SHADOW_NOT_FROZEN")
    if state.get("implementation_head_sha") != git("rev-parse", "HEAD"):
        raise ValueError("M12BE_CODE_CHANGED_AFTER_SHADOW_FREEZE")
    _configure_proof_runtime()
...
def _completion(
    *,
    ...
    completion = {
        ...
        "latest_result_zip_sha256": M12BD_BUNDLE_SHA256,
        ...
        "shadow_preflight_root_cause": (
            "SHADOW_ACTIVE_UNIVERSE_REPORT_COUNT_FIELD_NAME_DRIFT"
        ),
        "old_shadow_generation_id": M12BD_STOPPED_SHADOW_GENERATION_ID,
        "old_shadow_model_calls_started": 0,
        "old_shadow_model_calls_completed": 0,
        ...
        "stopped_shadow_offline_preflight_replay_status": program[
            "offline_replay"
        ]["status"],
        "task_start_active_monitor_count": len(active_tickers),
        "task_start_active_monitor_tickers": list(active_tickers),
...
def finalize_shadow() -> None:
    ...
    for slug in REPORT_SLUGS[70:101]:
    ...
    completion_reports = {
        "shadow-count-contract-repair-success-decision": {
            "status": "PASS" if matrix["status"] == "PASS" else "FAIL",
            "adapter_contract": COUNT_ADAPTER_CONTRACT,
            "direct_active_count_access_after": completion[
                "active_count_consumer_hardcode_count_after"
            ],
        },
        "m12bd-fictional-proof-reuse-success-decision": {
    ...
    if not clean:
        raise SystemExit("M12BE_SHADOW_HARD_ACCEPTANCE_FAILURE")
    print(
        json.dumps(
            {
                "status": "PASS",
...
def failure_closeout() -> None:
    stop = (
        ...
        if (PROOF_OUTPUT / "shadow/stop.json").is_file()
            ...
            "status": "BLOCKED",
            "stop_reason": "M12BE_UNCLASSIFIED_HARD_STOP",
        }
    )
...
def bundle(output_zip: Path) -> None:
    ...
    if missing:
        raise ValueError(f"M12BE_REQUIRED_REPORTS_MISSING:{missing}")
    ...
    index = {
        "contract": "m12be-artifact-index-v1",
    ...
    if index["status"] != "PASS":
        raise ValueError("M12BE_ARTIFACT_SECRET_SCAN_FAILURE")
    ...
    with zipfile.ZipFile(output_zip) as archive:
        if archive.testzip() is not None:
            raise ValueError("M12BE_RESULT_BUNDLE_INTEGRITY_FAILURE")
        for row in index["rows"]:
            payload = archive.read(row["path"])
            if hashlib.sha256(payload).hexdigest() != row["sha256"]:
                raise ValueError(f"M12BE_BUNDLE_HASH_MISMATCH:{row['path']}")
            if len(payload) != row["size"]:
                raise ValueError(f"M12BE_BUNDLE_SIZE_MISMATCH:{row['path']}")
