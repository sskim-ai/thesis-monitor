"""Archive-only M12W adapter over the existing instrumented CLI lifecycle."""
...
def single_attempt(
    *,
    ...
    ) != ("gpt-5.6-sol", "xhigh", 1800, 0, 4):
        raise ValueError("m12w_runtime_contract_mismatch")
    ...
    if any(
        ...
        raise ValueError("m12w_existing_artifact_refuses_rerun")
    ...
    receipt = {
        "contract": "m12w-single-attempt-receipt-v1",
        "invocation_id": invocation_id,
    ...
    if not readiness.ready:
        ...
        raise RuntimeError("m12w_network_preflight_failed")
    command = [
        codex_bin,
        "exec",
        "--ephemeral",
    ...
    success = (
        ...
        and observed == {"model": "gpt-5.6-sol", "effort": "xhigh"}
    )
    receipt.update(
    ...
    u.write(receipt_path, receipt)
    if not success:
        raise RuntimeError(f"m12w_model_context_failed:{receipt['failure_type']}")
    return receipt
