...
def test_selector_enum_and_contract_are_typed() -> None:
    assert CONTRACT_VERSION == "adaptive-renderer-selector-shadow-v1"
...
def test_hybrid_information_audit_detects_material_alternative_loss() -> None:
    ...
    audit = renderer_information_audit(
        INVENTORY_MESSAGE, analysis, AdaptiveRenderer.CONCISE_HYBRID
    )
    ...
    assert (
        ...
        in audit.material_dropped_elements
    )
...
def test_selected_renderer_never_drops_material_information() -> None:
    for benchmark_id, message in (
        ...
        assert decision.audit_for(
            decision.selected_renderer
...
def test_end_to_end_shadow_exercises_all_three_renderers() -> None:
    results = [
        run_adaptive_renderer_shadow(message, benchmark_id=benchmark_id)
        for benchmark_id, message in (
            ("inventory", INVENTORY_MESSAGE),
            ("fcf", FCF_MESSAGE),
            ("macro", MARKET_MESSAGE),
    ...
    assert all(
        result.final_delivery_mode == "ADAPTIVE_SHADOW_WOULD_SEND"
...
def test_selected_renderer_validator_parity() -> None:
    for benchmark_id, message in (
        ...
        result = run_adaptive_renderer_shadow(message, benchmark_id=benchmark_id)
...
def test_invalid_free_analyst_object_falls_back_without_free_message() -> None:
    ...
    assert result.final_delivery_mode == "VNEXT_SHADOW_FALLBACK"


def test_selector_failure_uses_safe_vnext_fallback() -> None:
...
def test_minimal_renderer_reuses_existing_safe_vnext() -> None:
    ...
    rendered = render_adaptive_candidate(
        MARKET_MESSAGE, analysis, AdaptiveRenderer.MINIMAL_VNEXT
    )
    result = run_adaptive_renderer_shadow(
        MARKET_MESSAGE, benchmark_id="minimal-equivalence"
    )

    assert rendered.text == result.final_text


def test_shadow_module_has_no_production_import_wiring() -> None:
    target = "adaptive_renderer_selector_shadow_service"
    importers = []
