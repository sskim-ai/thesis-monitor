...
def test_high_risk_financial_claims_fail_closed() -> None:
    ...
    assert result.explicit_not_fcf_disclaimer_count == 0
...
def test_ppe_proxy_affirmative_fcf_attributions_fail(text: str) -> None:
    proxy = _financial_ref(
        "ocf_less_ppe_capex",
        "352000000",
        derived=True,
    ...
    result = validate_directional_financial_semantics(
        {"claim": {"text": text, "evidence_refs": [proxy.ref_id]}},
        supplied_refs=(proxy,),
...
@pytest.mark.parametrize(
    "text",
    (
        "유형자산 취득 현금지출 차감 후 남는 현금입니다.",
        "현금 전환 대용치이며 관리 기준 잉여현금흐름이 아닙니다.",
        "free cash flow가 아닙니다.",
        "This is a cash-conversion proxy, not free cash flow.",
        "OCF less PPE acquisition cash outflow is positive.",
        "영업현금흐름 대부분을 유형자산 취득이 소모합니다.",
    ),
)
def test_ppe_proxy_descriptions_and_disclaimers_pass(text: str) -> None:
    proxy = _financial_ref(
        "ocf_less_ppe_capex",
        "352000000",
        derived=True,
...
def test_ppe_proxy_fcf_negation_is_clause_local() -> None:
    claims = classify_ppe_proxy_fcf_claims(
        "FCF가 352m인 것은 아니다, 하지만 사실상 FCF로 본다."
    )

    assert [claim.role for claim in claims] == [
        PPEProxyFCFClaimRole.EXPLICIT_NOT_FCF_DISCLAIMER,
        PPEProxyFCFClaimRole.PROXY_AS_FCF_ATTRIBUTION,
    ]


def test_ppe_proxy_cross_field_current_fcf_is_not_misattributed_to_proxy() -> None:
    proxy = _financial_ref(
        "ocf_less_ppe_capex",
        "352000000",
        derived=True,
