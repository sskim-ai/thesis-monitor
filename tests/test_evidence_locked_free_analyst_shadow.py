...
from app.services.ai_analyst_vnext_shadow_service import build_vnext_shadow_message
...
def test_hidden_arithmetic_is_rejected_even_when_raw_numbers_exist() -> None:
    ...
    assert result.status == "FAIL"
    assert "hidden_arithmetic_or_numeric_synthesis" in {
...
def test_novel_synthesis_is_distinct_from_current_and_vnext() -> None:
    analysis = _analysis()
    direct = render_free_analyst_direct(analysis)
    vnext = build_vnext_shadow_message(INVENTORY_MESSAGE)
...
def test_shadow_module_has_no_production_import_wiring() -> None:
    target = "evidence_locked_free_analyst_shadow_service"
    importers = []
...
def test_existing_vnext_behavior_remains_available_independently() -> None:
    before = build_vnext_shadow_message(INVENTORY_MESSAGE)
    analysis = _analysis()
    render_free_analyst_direct(analysis)
    after = build_vnext_shadow_message(INVENTORY_MESSAGE)
