...
def test_packet_builder_emits_typed_context_without_model_input_leak() -> None:
    ...
    contexts = {
        ref.financial_context.metric: ref
        for ref in enriched.evidence
    ...
    assert contexts["operating_cash_flow"].financial_context.evidence_status == (
        FinancialEvidenceStatus.DIRECT_REPORTED
    )
    assert contexts["ocf_less_ppe_capex"].financial_context.evidence_status == (
        FinancialEvidenceStatus.DERIVED_SAFE
    )
    assert contexts["ocf_less_ppe_capex"].label == "cash_conversion_ocf_less_ppe"
    assert contexts["ocf_less_ppe_capex"].metric_refs == ()
