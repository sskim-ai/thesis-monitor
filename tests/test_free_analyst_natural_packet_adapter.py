...
def test_stock_natural_packet_normalizes_to_common_free_analyst_contract() -> None:
    result = normalize_natural_packet_message(STOCK_MESSAGE, benchmark_id="stock", market="us")
...
def test_market_schedule_is_preserved_as_next_check() -> None:
    result = normalize_natural_packet_message(MARKET_MESSAGE, benchmark_id="market", market="us")

    assert result.status == "PASS"
    assert "📌 다음 확인\n향후 7일:" in result.normalized_text
    assert any(row.common_ref == "evidence:next_check:01" for row in result.evidence_ref_map)
...
def test_fourteen_message_fixture_shape_validates(index: int) -> None:
    message = MARKET_MESSAGE if index == 0 else STOCK_MESSAGE.replace("(MEM)", f"(T{index:02d})")
...
def test_evidence_ref_normalization_rejects_dangling_map() -> None:
    result = normalize_natural_packet_message(STOCK_MESSAGE, benchmark_id="dangling", market="kr")
...
def test_adapter_does_not_add_arithmetic_or_external_knowledge() -> None:
    ...
    result = normalize_natural_packet_message(original, benchmark_id="preserve-only", market="us")
...
def test_production_adapter_has_no_shadow_or_research_dependency() -> None:
    source = (ROOT / "app/services/free_analyst_natural_packet_adapter_service.py").read_text(
        encoding="utf-8"
    )
