...
def _by_path(fact: dict[str, object]) -> dict[str, dict[str, object]]:
    return {str(item["field_path"]): item for item in build_numeric_registry([fact])}
...
def test_sector_registry_has_one_owner_per_exact_supported_path() -> None:
    ...
    paths = [item["field_path"] for item in registry if str(item["field_path"]).endswith("_count")]

    assert len(paths) == len(set(paths)) == 6


def test_unknown_future_sector_count_remains_fail_closed() -> None:
    registry = build_numeric_registry([_sector_fact(experimental_component_count=7)])
    unknown = next(
        item for item in registry if item["field_path"] == "fields.experimental_component_count"
    )
