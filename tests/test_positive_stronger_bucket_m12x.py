...
def test_directional_prompt_and_fictional_source_case_are_unchanged():
    ...
    successor = x._freeze_paths(("scripts/directional_financial_context_m12.py",))
    assert successor["status"] == "FAIL"
    assert successor["change_count"] == 1
    assert successor["rows"][0]["path"] == "scripts/directional_financial_context_m12.py"
